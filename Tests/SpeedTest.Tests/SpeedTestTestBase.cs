...
namespace SpeedTest.Tests
{
    public abstract class SpeedTestTestBase : AbpIntegratedTestBase<SpeedTestTestModule>
    {
        ...
        protected void UsingDbContext(Action<SpeedTestDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SpeedTestDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SpeedTestDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SpeedTestDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SpeedTestDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SpeedTestDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SpeedTestDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SpeedTestDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SpeedTestDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SpeedTestDbContext, Task<T>> func)
        {
            T result;
